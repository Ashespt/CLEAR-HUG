...
def preprocess_signals(X_train, X_validation, X_test, outputfolder):
    ...
    if os.path.exists(standard_scaler_file):
        print('StandardScaler exists')
        with open(standard_scaler_file, 'rb') as ss_file:
            ss = pickle.load(ss_file)
        print("StandardScaler loaded from file.")
    else:
        print("StandardScaler does not exist. Creating new one.")
        ss = StandardScaler()
        ss.fit(np.vstack(X_train).flatten()[:, np.newaxis].astype(float))

        with open(standard_scaler_file, 'wb') as ss_file:
            pickle.dump(ss, ss_file)
        print("StandardScaler created and saved.")
