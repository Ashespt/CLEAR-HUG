...
class QRSTokenizer(nn.Module):
    ...
    def __init__(self, fs, max_len, token_len, lead_len, save_path, stage, used_chans=None):
    ...
    def plot_sequence(self, qrs_sequence, index,flag):
        fig, axs = plt.subplots(12, 15, figsize=(16, 16), constrained_layout=True)
        ...
        save_path = os.path.join(self.save_path, f"figs/QRS_Token_{index}_{flag}.png")
        plt.savefig(save_path)
        plt.close()
    ...
    def forward(self, x, plot=False):
        ...
        for batch in tqdm(range(bs), desc=f"Detect Batch QRS {self.stage}"):
            ...
            if batch in indexs and plot or not flag:
            ...
            if batch in indexs and plot or not flag:
                self.plot_sequence(qrs_sequence, batch,flag)
        ...
        batch_mask_pad = np.abs(1-np.array(batch_mask_pad).astype(np.int64))
        ...
        np.save(
            os.path.join(self.save_path, f"{self.stage}_data_mask_pad.npy"),
            batch_mask_pad,
        )
