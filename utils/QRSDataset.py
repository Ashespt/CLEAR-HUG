...
class ECGSingleDataset(Dataset):
    ...
    def __init_dataset(self) -> None:
        ...
        if 'zhongshan' in str(self.__folder_path).lower():
            print('Zhongshan data')
            self.data = self.data*2.44/1000
        ...
        print(os.path.join(str(self.__folder_path), self.__stage + "_data_mask_pad.npy"))
        if os.path.exists(os.path.join(str(self.__folder_path), self.__stage + "_data_mask_pad.npy")):
            self.in_mask_token = np.load(os.path.join(str(self.__folder_path), self.__stage + "_data_mask_pad.npy"))
        else:
            print('fail to fetch mask token!')
            self.in_mask_token = None
    ...
    def __getitem__(self, idx: int):
        if self.in_mask_token is not None:
            # return self.data_aug(self.data[idx]), self.in_chan_matrix[idx], self.in_time_matrix[idx], self.in_mask_token[idx]
            return self.data[idx], self.in_chan_matrix[idx], self.in_time_matrix[idx], self.in_mask_token[idx]
...
class ECGDatasetFinetune(Dataset):
    ...
    def __init__(
        ...
        self.mask_pad_path = os.path.join(
            data_folder_path, stage + "_data_mask_pad.npy"
        )
        self.data, self.labels, self.in_chan_matrix, self.in_time_matrix,self.mask_pad_matrix = (
            self.load_data()
        )
        self.prepare_data()
    ...
    def __getitem__(self, idx):
        ...
        mask_pad_matrix = self.in_time_matrix[idx]
        return sample, label, in_chan_matrix, in_time_matrix, mask_pad_matrix
    ...
    def load_data(self):
        ...
        if 'zhongshan' in str(self.data_path).lower():
            print('Zhongshan data')
            data = data*2.44/1000
        ...
        if os.path.exists(self.mask_pad_path):
            mask_pad = torch.from_numpy(np.load(self.mask_pad_path,allow_pickle=True))
        else:
            mask_pad = None
        return data, labels, in_chan_matrix, in_time_matrix,mask_pad
...
if __name__ == "__main__":
    ...
    train_dataset, test_dataset, val_dataset = prepare_finetune_dataset("./datasets/ecg_datasets/PTBXL_QRS_12Leads/rhythm",0.01,"random")
    ...
    train_dataset, test_dataset, val_dataset = prepare_finetune_dataset("./datasets/ecg_datasets/PTBXL_QRS_12Leads/rhythm",0.1,"random")
    data_loader_val = torch.utils.data.DataLoader(val_dataset,batch_size=int(1.5 * 2),drop_last=False,)
    for batch in data_loader_val:
        print(batch[0].shape)
        print(batch[1].shape)
