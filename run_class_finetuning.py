...
def get_args():
    ...
    parser.add_argument(
        "--cls_token_num", default=1, type=int, metavar="N", help="#cls token"
    )
...
def get_models(args):
    model = create_model(args.model, pretrained=False, num_classes=args.nb_classes, cls_token_num=args.cls_token_num,padding_mask=args.padding_mask,atten_mask=args.atten_mask,mask_ratio=args.mask_ratio)
    return model
